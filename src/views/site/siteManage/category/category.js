  ...
  methods: {
    ...
    toggleCate(data) {
      ...
      if (data.level == 1) {
        ...
        _this.getSecCateList(_this.secondData.id);
        if (data.childList.length > 0) {
    ...
    addCategory(flag) {
      ...
      let _this = this;
      _this
        .$prompt('请输入' + (flag == 1 ? '一级' : '二级') + '分类名', '提示', {
    ...
    delCategory(delData) {
      ...
      let _this = this;
      _this
        .$confirm('确认删除分类:' + delData.name + '?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
    ...
    matchData() {
      //提交勾选了的分类
      let _this = this,
        params = { siteId: _this.siteId, sourceTypeIdList: _this.currCheckList },
        currObj;
      for (let i of _this.secondData.childList) {
        if (i.active) {
          currObj = i;
      ...
      if (currObj) {
        params.siteTypeId = currObj.id;
      } else {
        params.siteTypeId = _this.secondData.id;
      }
      _this.$sendData('post', 'showSiteType/addTypeMap', params, (data, all) => {
        if (currObj && currObj.level == 2) {
